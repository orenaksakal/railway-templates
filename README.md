...
## Additional unpublished drafts

Ten more candidates are prepared separately: Appwrite, AppFlowy Cloud, DBOS, SpiceDB, Frappe CRM, Frappe Helpdesk, Matrix with MAS, Immich, Novu, and RAGFlow. See [DRAFTS.md](DRAFTS.md) for sources, checks, and unresolved compatibility gates. Their configurations are generated by `scripts/catalog_round3.py`; the original five listings remain separate.
