...
for attempt in $(seq 1 90); do
  if mongo --quiet --host 127.0.0.1 --username "$MONGO_INITDB_ROOT_USERNAME" --password "$MONGO_INITDB_ROOT_PASSWORD" --authenticationDatabase admin --eval 'try { rs.status(); } catch (e) { if (e.code !== 94) throw e; rs.initiate({_id:"rs0",members:[{_id:0,host:_getEnv("MONGO_PRIVATE_HOST")+":27017"}]}); }' >/dev/null 2>&1; then
    ready=true; break
  fi
  sleep 2
done
