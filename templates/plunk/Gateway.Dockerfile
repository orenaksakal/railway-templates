...
COPY templates/plunk/gateway.conf.template /etc/nginx/templates/default.conf.template
COPY shared/round6-gateway/proxy.inc /etc/nginx/proxy.inc
ENV NGINX_ENVSUBST_FILTER="^(DNS_RESOLVER|UPSTREAM_HOST|UPSTREAM_PORT|AUTH_REALM|ACCESS_PASSWORD|OWNER_SCOPE)$"
ENTRYPOINT ["/start.sh"]
