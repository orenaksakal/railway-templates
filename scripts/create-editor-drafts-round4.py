...
def main():
    ...
    root=ROOT/'.local/round4';root.mkdir(parents=True,exist_ok=True);path=root/'drafts.json';receipts=json.loads(path.read_text()) if path.exists() else {}
    def save():
        temporary=path.with_suffix('.tmp');temporary.write_text(json.dumps(receipts,indent=2)+'\n');temporary.replace(path)
    metadata=json.loads((ROOT/'marketplace.round4.json').read_text())
