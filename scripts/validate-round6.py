"""Bounded static checks for the unpublished round-six catalog."""
import ast, json, re, subprocess
from pathlib import Path
from catalog_round6 import ROOT,CATALOG
errors=[];count=0
for name,factory in CATALOG.items():
 p=ROOT/'templates'/name
 config=json.loads((p/'template.json').read_text());assert list(config['services'].values())==factory(),name+' stale generated config';services=list(config['services'].values());byname={s['name']:s for s in services}
 ...
 for service in services:
  ...
  if 'image' in source:assert re.search(r'@sha256:[0-9a-f]{64}$',source['image']),label
   ...
   assert source['branch']=='codex/fifteen-template-drafts'
...
for f in (ROOT/'shared').glob('round6-*/*.sh'):subprocess.run(['sh','-n',str(f)],check=True)
print(f'PASS: {len(CATALOG)} drafts, {count} services; pins, references, sources, descriptions, volumes and script syntax')

for metadata in json.loads((ROOT/"marketplace.round6.json").read_text()).values():
...
for f in (ROOT/'shared').glob('round6-*/*.py'):
    ast.parse(f.read_text())
locks=json.loads((ROOT/'images.round6.lock.json').read_text())
for tag,image in locks.items():
    assert image.startswith(tag+'@sha256:') and re.fullmatch(r'.+@sha256:[0-9a-f]{64}',image),tag
sources=json.loads((ROOT/'sources.round6.lock.json').read_text())
assert set(CATALOG)<=set(sources)
for name in CATALOG:
    assert re.fullmatch(r'[0-9a-f]{40}',sources[name]['sha']),name
print('PASS: source receipts, image lock structure and shared Python syntax')
