...
spec=importlib.util.spec_from_file_location('editor',ROOT/'scripts/create-editor-drafts-round6.py')
...
def main():
 ...
 os.umask(0o077)
 path=ROOT/'.local/round6/drafts.json'
 ...
 for slug in CATALOG:
  ...
  if not args.verify_only and (live['status']!='PUBLISHED' or any(live.get(k)!=v for k,v in expected.items())):
   ...
   (ROOT/'.local/round6'/f'{slug}-publish.json').write_text(result.stdout)
   live=read()
  ...
  (ROOT/'.local/round6'/f'{slug}-published-readback.json').write_text(json.dumps(live,indent=2)+'\n')
  save()
