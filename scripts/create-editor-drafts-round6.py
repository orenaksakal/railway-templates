"""Create/update the fifteen UNPUBLISHED drafts without projects or deployments.
...
def main():
    ...
    root=ROOT/'.local/round6';root.mkdir(parents=True,exist_ok=True);path=root/'drafts.json';receipts=json.loads(path.read_text()) if path.exists() else {}
    def save():
        temporary=path.with_suffix('.tmp');temporary.write_text(json.dumps(receipts,indent=2)+'\n');temporary.replace(path)
    metadata=json.loads((ROOT/'marketplace.round6.json').read_text())
    for name in ([args.template] if args.template else CATALOG):
        ...
        if live['status'] != 'UNPUBLISHED' or normalized_config(actual)!=normalized_config(config) or any(live.get(k)!=v for k,v in meta.items()):raise RuntimeError(name+': saved configuration or metadata did not match')
