...
CATALOG = json.loads((ROOT/'marketplace.round9.json').read_text())
...
def main():
 ...
 os.umask(0o077)
 path=ROOT/'.local/round9/drafts.json'
 ...
 if len({r['templateId'] for r in receipts.values()})!=18:raise RuntimeError('Duplicate template IDs')
 for slug in CATALOG:
 ...
 print('PASS: complete batch preflight, 18 templates / 38 services',flush=True)
 if args.preflight_only:return
 for slug in CATALOG:
  receipt=receipts[slug]
  config=json.loads((ROOT/'templates/round9'/slug/'template.json').read_text())
  readme=ROOT/'templates/round9'/slug/'README.md'
  ...
  if not args.verify_only and (live['status']!='PUBLISHED' or any(live.get(k)!=v for k,v in expected.items())):
   ...
   (ROOT/'.local/round9'/f'{slug}-publish.json').write_text(result.stdout)
   live=read()
  ...
  (ROOT/'.local/round9'/f'{slug}-published-readback.json').write_text(json.dumps(live,indent=2)+'\n')
  save()
