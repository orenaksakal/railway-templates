"""Create/update the twenty qualifying UNPUBLISHED drafts without projects or deployments.
...
def main():
    ...
    root=ROOT/'.local/round8';root.mkdir(parents=True,exist_ok=True);path=root/'drafts.json';receipts=json.loads(path.read_text()) if path.exists() else {}
    def save():
