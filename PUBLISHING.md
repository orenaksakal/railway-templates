...
2. Prefer the existing unpublished drafts linked in `VALIDATION.md`; all five configurations have been verified against the catalog. Use the committed `codex/railway-template-release` source branch for these listings. For replacement drafts, create them directly in the Railway template editor to avoid source-project deployments. Alternatively, create unpublished drafts using `scripts/create-drafts.py --stage-only` only when source-project deployments and their usage are authorized. Restore constant variable defaults in each service's Raw Editor, then **Apply** the staged changes on the template canvas. Run `--verify-only` against the saved drafts. Inspect each draft's services, Dockerfile paths, ports, variables, and volumes against its `template.json`.
